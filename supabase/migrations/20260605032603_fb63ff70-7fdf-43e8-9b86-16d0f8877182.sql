...
BEGIN
  ...
  _leads_agg AS (
    ...
    LEFT JOIN public.leads l ON l.tenant_id = m.tenant_id AND l.assigned_to = m.user_id
  ...
  _appt_agg AS (
    SELECT a.consultant_member_id AS mid,
           ...
           COUNT(*) FILTER (WHERE a.status = 'completed')::bigint AS done
  ...
  SELECT m.id, m.display_name, m.avatar_color, m.avatar_url, m.role_label,
         GREATEST(
           c.legacy_pts,
           -- Pontos só por trabalho efetivo: contatos, reuniões, vendas e respostas rápidas.
           -- "Leads recebidos" NÃO geram pontos (é só volume distribuído, não mérito).
           (c.sls * _p_sale + c.mts * _p_meet + c.cts * _p_contact + c.frs * _p_fast)
         )::bigint AS points,
