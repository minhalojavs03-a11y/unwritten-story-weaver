...
AS $function$
  WITH allowed AS (
    ...
    WHERE (a.is_superadmin OR public.get_tenant_role(auth.uid(), tm.tenant_id) = 'owner'::tenant_role)
      ...
      -- Para superadmin (visão global), inclui owners como "consultor único" do tenant.
      -- Para owners (visão do próprio tenant), continua excluindo a si próprio.
      AND (a.is_superadmin OR tm.role <> 'owner'::tenant_role)
      AND (a.is_superadmin OR lower(coalesce(p.role_label, '')) NOT IN ('dono', 'owner', 'proprietário', 'proprietario', 'superadmin'))
    ...
    WHERE d.is_active = true
      ...
      AND (a.is_superadmin OR lower(coalesce(d.role_label, '')) NOT IN ('dono', 'owner', 'proprietário', 'proprietario', 'superadmin'))
  )
  SELECT
    d.id,
    ps.user_id,
