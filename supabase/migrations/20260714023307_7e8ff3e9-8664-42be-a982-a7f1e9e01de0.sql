...
BEGIN
  ...
  -- Pool base (elegibilidade por role/origem/tier), com contagem do dia e prioridade.
  WITH base AS (
    SELECT
      tm.id,
      tm.user_id,
  ...
  -- Fallback 1: ignora conexão de WhatsApp mas mantém tier e teto.
  ...
  -- Fallback 2: ignora teto diário (mas respeita tier e origem).
  -- Distribui para quem tem MENOS leads hoje, para não sobrecarregar ninguém.
  IF _picked IS NULL THEN
    WITH base AS (
      SELECT
        tm.id, tm.user_id,
  ...
  -- Fallback 3: ignora tier também. Pega qualquer consultor da origem correta
  -- com menor carga do dia. Lead NUNCA fica sem consultor.
  IF _picked IS NULL THEN
    ...
    ORDER BY
      (SELECT COUNT(*) FROM public.leads l
       WHERE l.assigned_member_id = tm.id
         AND (l.assigned_member_at AT TIME ZONE 'America/Sao_Paulo')::date
             = (now() AT TIME ZONE 'America/Sao_Paulo')::date) ASC,
      random()
    LIMIT 1;
  END IF;

  -- Fallback 4: qualquer consultor ativo (última rede de segurança).
  IF _picked IS NULL THEN
    SELECT tm.id, tm.user_id INTO _picked, _picked_user
    FROM public.tenant_members tm
    WHERE tm.tenant_id = NEW.tenant_id
