...
BEGIN
  ...
  WITH _lead_events AS (
    SELECT m.id AS mid,
           ...
           -- Reunião conta quando foi AGENDADA (anotação do consultor),
           -- independentemente do cliente ter comparecido ou fechado.
           (
             l.stage IN ('compareceu','comprou')
             OR NULLIF(l.metadata->>'meeting_scheduled_at','') IS NOT NULL
             OR NULLIF(l.metadata->>'meeting_rescheduled_at','') IS NOT NULL
             OR NULLIF(l.metadata->>'meeting_closer_id','') IS NOT NULL
             OR lower(COALESCE(l.lead_phase,'')) IN ('reuniao','reunião','agendado_reuniao')
             OR EXISTS (
               SELECT 1 FROM public.lead_stage_events se
               WHERE se.lead_id = l.id
                 AND se.tenant_id = l.tenant_id
                 AND (se.label ILIKE '%reuni%agend%' OR se.label ILIKE '%compareceu%' OR se.label ILIKE '%reagend%')
             )
           ) AS is_meeting,
           (
             COALESCE(NULLIF(l.metadata->>'sale_date','')::timestamptz, l.updated_at) >= _s
             OR l.updated_at >= _s
           ) AS sale_in_period,
           (
             COALESCE(
               NULLIF(l.metadata->>'meeting_rescheduled_at','')::timestamptz,
               NULLIF(l.metadata->>'meeting_scheduled_at','')::timestamptz,
               l.updated_at
             ) >= _s
             OR l.updated_at >= _s
           ) AS meeting_in_period,
           (l.last_contact_at >= _s) AS contact_in_period,
           (COALESCE(l.assigned_member_at, l.updated_at) >= _s) AS assumed_in_period
  ...
  _leads_agg AS (
    SELECT mid,
           COUNT(*) FILTER (WHERE is_sale AND sale_in_period)::bigint AS sls,
           COUNT(*) FILTER (WHERE contact_in_period)::bigint AS cts,
           COUNT(*) FILTER (WHERE assumed_in_period)::bigint AS las
    FROM _lead_events
    GROUP BY mid
  ),
  _meeting_units AS (
    ...
    FROM _lead_events
    WHERE is_meeting AND meeting_in_period
    UNION
