...
BEGIN
  ...
  -- Detecta a origem (Leads 01 vs Leads 02) a partir do metadata gravado pelo sheets-sync
  ...
  WHERE tm.tenant_id = NEW.tenant_id
    ...
    AND (
      (_is_leads02 AND tm.receives_leads_02 = true)
      OR (NOT _is_leads02 AND tm.receives_leads = true)
    )
    ...
    AND (
      _credit IS NULL
      OR tm.min_credit_value IS NULL
      OR _credit >= tm.min_credit_value
    )
    AND EXISTS (
