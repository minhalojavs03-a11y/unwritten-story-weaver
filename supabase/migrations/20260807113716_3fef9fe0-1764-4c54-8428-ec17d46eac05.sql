...
BEGIN
  ...
  -- 1) PRIORIDADE ABSOLUTA: Micaelly, Nilton e David primeiro (até cota diária).
  WITH candidates AS (
    SELECT
      tm.id,
      tm.user_id,
    ...
    WHERE tm.tenant_id = NEW.tenant_id
      ...
      AND (
        ...
        OR lower(coalesce(tm.display_name, '')) LIKE '%nilton%'
        OR lower(coalesce(tm.display_name, '')) LIKE '%david%'
        OR lower(coalesce(tm.username, '')) LIKE '%nilton%'
        OR lower(coalesce(tm.username, '')) LIKE '%ilton_%'
  ...
  -- 2) Rotação normal (Diéssica fica de fora nesta etapa).
  IF _picked IS NULL THEN
    WITH candidates AS (
      ...
      WHERE tm.tenant_id = NEW.tenant_id
        ...
        AND lower(coalesce(tm.role_label, '')) NOT LIKE '%dono%'
        AND lower(coalesce(tm.display_name, '')) NOT LIKE '%teste%'
        AND lower(coalesce(tm.display_name, '')) NOT LIKE '%diessica%'
        AND lower(coalesce(tm.display_name, '')) NOT LIKE '%diéssica%'
        AND ((_credit IS NULL) OR tm.max_credit_value IS NULL OR _credit <= tm.max_credit_value)
        AND ((_credit IS NULL) OR tm.min_credit_value IS NULL OR _credit >= tm.min_credit_value)
    )
    SELECT id, user_id INTO _picked, _picked_user
  ...
  -- 2b) Diéssica: só entra quando ninguém mais está abaixo da cota.
  IF _picked IS NULL THEN
    ...
    WHERE tm.tenant_id = NEW.tenant_id
      AND tm.is_active = true
      AND ((_is_leads02 AND tm.receives_leads_02 = true) OR (NOT _is_leads02 AND tm.receives_leads = true))
      AND (lower(coalesce(tm.display_name, '')) LIKE '%diessica%' OR lower(coalesce(tm.display_name, '')) LIKE '%diéssica%')
      AND ((_credit IS NULL) OR tm.max_credit_value IS NULL OR _credit <= tm.max_credit_value)
      AND ((_credit IS NULL) OR tm.min_credit_value IS NULL OR _credit >= tm.min_credit_value)
      AND (SELECT count(*) FROM public.leads l
            WHERE l.tenant_id = tm.tenant_id
              AND l.kind = 'lead'
              AND l.assigned_member_id = tm.id
              AND l.assigned_member_at >= _sp_midnight) < COALESCE(tm.daily_lead_limit, 1)
    ORDER BY random()
    LIMIT 1;
  END IF;

  -- 3) Fallback: qualquer consultor ativo e habilitado, ignorando cota.
  IF _picked IS NULL THEN
    SELECT tm.id, tm.user_id INTO _picked, _picked_user
    FROM public.tenant_members tm
    WHERE tm.tenant_id = NEW.tenant_id
...
SET daily_lead_limit = 5
WHERE lower(display_name) LIKE '%micaelly%' OR lower(display_name) LIKE '%nilton%';