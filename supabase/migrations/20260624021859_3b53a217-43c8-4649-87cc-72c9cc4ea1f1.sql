...
ALTER TABLE public.tenant_members
  ADD COLUMN IF NOT EXISTS receives_leads_02 BOOLEAN NOT NULL DEFAULT false;

DROP FUNCTION IF EXISTS public.list_distribution_consultants(uuid);

CREATE FUNCTION public.list_distribution_consultants(_tenant_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(id uuid, user_id uuid, tenant_id uuid, display_name text, username text, role_label text, avatar_url text, avatar_color text, is_active boolean, receives_leads boolean, receives_leads_02 boolean, min_credit_value numeric, max_credit_value numeric, daily_lead_limit integer, notify_inapp boolean, notify_whatsapp boolean, phone text)
 LANGUAGE sql
...
CREATE OR REPLACE FUNCTION public.update_member_distribution_v2(
  _member_id uuid,
...
BEGIN
  ...
  IF NOT public.has_app_role(auth.uid(), 'superadmin'::app_role)
     AND v_role NOT IN ('owner'::tenant_role) THEN
  ...
  UPDATE public.tenant_members
     SET receives_leads = _receives_leads_01,
         receives_leads_02 = _receives_leads_02,
...
GRANT EXECUTE ON FUNCTION public.update_member_distribution_v2(uuid, boolean, boolean, numeric, numeric, integer) TO authenticated;
